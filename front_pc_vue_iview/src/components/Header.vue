    ...
    <Row type="flex" justify="center">
      <Col span="18" style="display:flex;justify-content: space-between;">
        ...
        >
          ...
          <span
            style="font-size:12px;line-height:50px;color:gray;text-indent: 8px;"
          >&nbsp;&nbsp;Full Stack Web Developer</span>
        </router-link>

        <Menu mode="horizontal" theme="light" active-name="1" @on-select="name=>{name=='logout' && logout()}">
          <div class="layout-nav">
            <MenuItem v-for="ele in types" :key="ele.id" :to="`/blogs/${ele.id}`" :name="ele.id" style="display:flex;align-items: center;">
